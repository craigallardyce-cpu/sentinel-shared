...
function watchWarnings(nightManoeuvre, legs) {
    ...
    const nightWork = legs.filter((l, i) => l.manoeuvre && i > 0 && isNightAt(l.lat, l.lon, Date.parse(legs[i - 1].time))).length;
...
export function routeIsochrone(options) {
    ...
    for (let step = 0; step < maxSteps; step++) {
        ...
        for (const node of frontier) {
            ...
            const sailedOn = (headingDeg, twaDeg) => inSea(boatSpeed(polar, twaDeg, windForSails.speedKts), headingDeg);
            /**
