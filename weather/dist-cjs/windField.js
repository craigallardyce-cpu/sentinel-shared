...
exports.boundsForPassage = boundsForPassage;
exports.toComponents = toComponents;
exports.fromComponents = fromComponents;
exports.fetchWindField = fetchWindField;
exports.createWindSampler = createWindSampler;
/**
...
function boundsForPassage(start, destination, marginDeg = 2) {
    return {
...
function toComponents(speedKts, directionFromDeg) {
 ...
 */
async function fetchWindField(bounds, options = {}) {
    ...
    if (points.length !== lats.length * lons.length) {
        throw new Error(`Open-Meteo returned ${points.length} forecast points for a ${lats.length}x${lons.length} grid.`);
    }
    const timeStrings = points[0]?.hourly?.time ?? [];
    if (!timeStrings.length)
        throw new Error('Open-Meteo wind field contained no hourly data.');
 ...
 */
function createWindSampler(field) {
    return (lat, lon, timeMs) => {
        ...
        const blend = (ti) => {
            ...
            if (values.some((c) => !Number.isFinite(c.u) || !Number.isFinite(c.v)))
                return null;
            const low = {
