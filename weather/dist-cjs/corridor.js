...
exports.scanHazards = scanHazards;
exports.buildCorridor = buildCorridor;
exports.buildAdvisory = buildAdvisory;
exports.compareToPlan = compareToPlan;
 ...
 */
function scanHazards(route, samplers, options = {}) {
...
const fronts = (route) => route.fronts ?? [];
...
function spreadNm(points) {
    ...
    for (let i = 0; i < points.length; i++) {
        for (let j = i + 1; j < points.length; j++) {
            const d = (0, routing_js_1.distanceNm)(points[i].lat, points[i].lon, points[j].lat, points[j].lon);
            if (d > worst)
                worst = d;
        }
    }
 ...
 */
function buildCorridor(route, destination, options = {}) {
    const bands = [];
    ...
    const bestRemainingAt = (timeMs) => {
        ...
        return closest
            ? (0, routing_js_1.distanceNm)(closest.lat, closest.lon, destination.lat, destination.lon)
            : null;
    };
    for (const front of fronts(route)) {
        ...
        const worthwhile = front.points.filter((p) => {
            ...
            if (baseline === null)
                return false;
            const remaining = (0, routing_js_1.distanceNm)(p.lat, p.lon, destination.lat, destination.lon);
...
function buildAdvisory(route, scan, corridor, filedEtaHours = null) {
 ...
 */
function compareToPlan(checkpoints, samplers, options = {}) {
