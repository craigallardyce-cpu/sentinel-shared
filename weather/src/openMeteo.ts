 ...
 */
const NWS_COVERAGE_BOXES: ReadonlyArray<readonly [number, number, number, number]> = [
...
export function isInsideNwsCoverage(lat: number, lon: number): boolean {
  const latNum = Number(lat);
  const lonNum = Number(lon);
  if (!Number.isFinite(latNum) || !Number.isFinite(lonNum)) return false;
  return NWS_COVERAGE_BOXES.some(
    ([minLat, maxLat, minLon, maxLon]) =>
      latNum >= minLat && latNum <= maxLat && lonNum >= minLon && lonNum <= maxLon
  );
}
...
export function formatPosition(lat: number, lon: number): string {
  const latNum = Number(lat);
  const lonNum = Number(lon);
  if (!Number.isFinite(latNum) || !Number.isFinite(lonNum)) return 'Unknown position';
 ...
 * backwards on a boat.
 */
function meanDirection(degrees: number[]): number | null {
  const valid = degrees.filter((d): d is number => Number.isFinite(d));
...
function assessRisk(input: {
  sustainedKts: number | null;
  gustKts: number | null;
  waveMetres: number | null;
}): 'low' | 'moderate' | 'high' {
  let risk: 'low' | 'moderate' | 'high' = 'low';
  const raise = (level: 'moderate' | 'high') => {
  ...
  if (sustainedKts !== null) {
    if (sustainedKts > 25) raise('high');
    else if (sustainedKts > 15) raise('moderate');
  }
  if (gustKts !== null) {
    if (gustKts > 34) raise('high');
    else if (gustKts > 22) raise('moderate');
  }
  if (waveMetres !== null) {
    if (waveMetres > 4) raise('high');
    else if (waveMetres > 2.5) raise('moderate');
  }
  return risk;
}

function describePressureTrend(pressures: unknown[]): string | null {
  const valid = finiteOnly(pressures);
  if (valid.length < 2) return null;
  const change = valid[valid.length - 1] - valid[0];
  const hPa = Math.round(valid[valid.length - 1]);
  if (change <= -3) return `${hPa} hPa falling`;
  if (change >= 3) return `${hPa} hPa rising`;
 ...
 * wind forecast.
 */
async function fetchMarineHourly(lat: number, lon: number): Promise<MarineHourly | null> {
  try {
    const url =
