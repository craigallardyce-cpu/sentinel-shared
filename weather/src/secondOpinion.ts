...
const earliest = (outcomes: DepartureOutcome[]): DepartureOutcome | null => {
  let best: DepartureOutcome | null = null;
  for (const o of outcomes) {
    if (!o.reached) continue;
    // Arrival time, not passage length: the question is who gets there first.
    if (!best || o.key + o.etaHours < best.key + best.etaHours) best = o;
  }
  return best;
};
 ...
 */
export function compareSecondOpinion(
  primary: DepartureOutcome[],
  second: DepartureOutcome[],
  model: string,
  modelLabel = model
): SecondOpinion | null {
  if (!primary?.length || !second?.length) return null;

  const primaryBest = earliest(primary);
  const secondBest = earliest(second);

  if (!primaryBest) return null; // The primary found nothing; nothing to second-guess.
  ...
  if (!secondForPrimaryPick?.reached) {
    return {
      ...
      etaDeltaHours: null,
      summary:
        `${modelLabel} does not get this passage in at all on the departure that arrives first here. ` +
  ...
  if (sameBest && !material) {
    return {
      ...
      etaDeltaHours: delta,
      summary:
        `${modelLabel} agrees: same departure, and its arrival is within ${hours(delta)} of this one.`
    };
  }
  ...
  if (sameBest) {
    return {
      ...
      etaDeltaHours: delta,
      summary:
        `${modelLabel} picks the same departure but puts the arrival ${hours(delta)} ` +
  ...
  return {
    ...
    etaDeltaHours: delta,
    summary:
      `${modelLabel} has a different departure arriving first, and puts this one ${hours(delta)} ` +
