...
export async function fetchEntitlements(
  ...
  const addTier = (tier: any) => {
    if (tier?.product_id !== productId || typeof tier?.id !== 'string') return;
  ...
  (subs ?? []).forEach((s: any) => addTier(s.tiers));
  ...
  const bundleTierIds = (bundles ?? []).map((b: any) => b.bundle_tier_id).filter(Boolean);
  if (bundleTierIds.length > 0) {
    ...
    (mappings ?? []).forEach((m: any) => addTier(m.tiers));
  }
