...
export function updateFeedUrl(app: FleetApp, base: string = UPDATE_FEED_BASE): string {
...
export type AppVersionResult =
  | { ok: true; currentVersion: string; latestVersion: string; hasUpdate: boolean; changelog: string }
 ...
 * can act on.
 */
export async function checkLatestRelease(opts: CheckLatestReleaseOptions): Promise<AppVersionResult> {
  const { app, currentVersion } = opts;
  const timeoutMs = opts.timeoutMs ?? DEFAULT_TIMEOUT_MS;
  const fetchImpl = opts.fetchImpl ?? globalThis.fetch;
  const failed: AppVersionResult = { ok: false, currentVersion, error: UNREACHABLE };
  ...
  try {
    ...
    if (typeof version !== 'string' || !VERSION_SHAPE.test(version.trim())) return failed;
    if (notes !== undefined && !Array.isArray(notes)) return failed;

    const latestVersion = version.trim().replace(/^v/i, '');
    const changelog = (Array.isArray(notes) ? notes : []).filter((n): n is string => typeof n === 'string').join('\n');
    return {
      ok: true,
      currentVersion,
      latestVersion,
 ...
 */
export function appVersionHandler(
  opts: AppVersionHandlerOptions,
): (req: unknown, res: AppVersionResponse) => Promise<void> {
