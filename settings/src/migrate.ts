 ...
 */
export async function migrateLegacyKeys<D extends Record<string, AnySpec>>(
  options: MigrateLegacyOptions<D>
  ...
  const result: MigrateLegacyResult = { migrated: {}, unparseable: [], alreadyDone: false };
  ...
  for (const definition of registry.all()) {
    ...
    */
    ...
    if (settings.isConfigured(definition.key)) continue;
