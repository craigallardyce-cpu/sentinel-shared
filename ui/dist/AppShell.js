...
export function HeaderButton({ icon, active = false, label, className, ...rest }) {
    return (_jsxs("button", { type: "button", className: cn('flex items-center gap-1.5 font-mono text-[11px] font-bold tracking-widest transition-colors duration-150 cursor-pointer min-h-8', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan/60 rounded-md px-1', active ? 'text-cyan drop-shadow-[0_0_8px_var(--color-cyan-glow)]' : 'text-text-muted hover:text-text-primary', className), "aria-pressed": active, ...rest, children: [_jsx("span", { className: "shrink-0", "aria-hidden": true, children: icon }), label && _jsx("span", { className: "hidden sm:inline", children: label })] }));
}
