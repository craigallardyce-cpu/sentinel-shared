 ...
 */
export function SettingsSection({ title, icon, description, children, className, hideTitle = false }) {
    return (_jsxs("section", { className: cn('space-y-3', className), "aria-label": hideTitle && typeof title === 'string' ? title : undefined, children: [(!hideTitle || description) && (_jsxs("header", { children: [!hideTitle && (_jsxs("h3", { className: "flex items-center gap-2 text-[13px] font-mono font-bold uppercase tracking-wider text-cyan", children: [icon && _jsx("span", { className: "shrink-0", "aria-hidden": true, children: icon }), title] })), description && _jsx("p", { className: cn('text-xs text-text-muted', !hideTitle && 'mt-1'), children: description })] })), _jsx("div", { className: "space-y-3", children: children })] }));
}
/**
...
export function SettingsShell({ open, onClose, appName, appIcon, version, nightMode, onNightModeChange, dayBrightness, onDayBrightnessChange, nightBrightness, onNightBrightnessChange, keepAwake, onKeepAwakeChange, updater, children, tabs, displayExtra, footer, size = 'lg', about, summary, sources, title = 'Settings', }) {
    ...
    const combinedAbout = updatesSection ? (_jsxs("div", { className: "space-y-8", children: [renderAbout({ appName, appIcon, shownVersion: undefined, about }, inTabs), updatesSection] })) : inTabs ? renderAbout({ appName, appIcon, shownVersion, about }, true) : aboutSection;
    const allTabs = tabs
        ? [
