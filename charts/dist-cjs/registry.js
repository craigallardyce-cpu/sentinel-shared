...
exports.tileBboxMeters = tileBboxMeters;
exports.maritimeChartService = maritimeChartService;
exports.createChartRegistry = createChartRegistry;
...
const NOAA_MCS = 'https://gis.charttools.noaa.gov/arcgis/rest/services/MCS/ENCOnline/MapServer/exts/MaritimeChartService/MapServer';
const CHS_MCS = 'https://egisp.dfo-mpo.gc.ca/arcgis/rest/services/chs/ENC_MaritimeChartService/MapServer/exts/MaritimeChartService/MapServer';
/**
 ...
 */
function maritimeChartService(baseUrl) {
    return (z, x, y, opts = {}) => {
        const displayParams = {
            ECDISParameters: {
                DynamicParameters: {
...
function buildProviders(options) {
    ...
    return [
        ...
        },
        {
            ...
            attribution: '© MapTiler © OpenStreetMap contributors',
            maxZoom: 20,
            build: (z, x, y) => `https://api.maptiler.com/maps/streets-v2/256/${z}/${x}/${y}.png?key=${key}`
        },
        {
            id: 'satellite',
            label: 'Satellite',
            ...
            build: (z, x, y) => `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${z}/${y}/${x}`
        },
        {
            id: 'osm',
            label: 'Shore',
...
function coversPoint(provider, lat, lon) {
    ...
    if (!provider.coverage)
        return true;
    return provider.coverage.some((b) => lon >= b.west && lon <= b.east && lat >= b.south && lat <= b.north);
}
/**
 ...
 */
function createChartRegistry(options = {}) {
    ...
    const active = all.filter((p) => hasLicensedBasemap ? !UNLICENSED_IDS.has(p.id) : !MAPTILER_IDS.has(p.id));
    const byId = new Map(active.map((p) => [p.id, p]));
    const fallbackId = options.fallbackId ?? (hasLicensedBasemap ? 'maptiler_satellite' : 'satellite');
    const getProvider = (id) => byId.get(id) ?? null;
    return {
        all,
        active,
        hasLicensedBasemap,
        getProvider,
        selectBaseProvider(lat, lon) {
            if (typeof lat !== 'number' || typeof lon !== 'number' || isNaN(lat) || isNaN(lon)) {
                return getProvider(fallbackId);
            }
            const official = active.find((p) => p.kind === 'base' && p.authority === 'official' && coversPoint(p, lat, lon));
