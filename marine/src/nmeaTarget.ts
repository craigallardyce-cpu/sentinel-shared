...
export interface ResolveNmeaTargetInput {
  ...
  requested?: { host?: string | null; port?: string | number | null };
  /**
   ...
   */
  configured?: { host?: string | null; port?: string | number | null } | null;
  ...
  fallback?: { host: string; port: string };
}
 ...
 */
export function splitPoolKey(key: string): { host: string; port: string } | null {
  const at = key.lastIndexOf(':');
  if (at <= 0 || at === key.length - 1) return null;
  return { host: key.slice(0, at), port: key.slice(at + 1) };
}

export function resolveNmeaTarget(input: ResolveNmeaTargetInput = {}): NmeaTarget {
  ...
  if (reqHost && reqPort) return { host: reqHost, port: reqPort, source: 'requested' };
  ...
  */
  for (const key of input.activeKeys ?? []) {
    ...
    if (split) return { ...split, source: 'pool' };
  }
 ...
 */
export function stalePoolKeys(activeKeys: Iterable<string>, current: { host: string; port: string }): string[] {
