...
import { parseAisSentence, type AisTargetData } from './ais.js';

export const liveAisTargetsMap = new Map<string, AisTargetData>();

/** Auto-detected own vessel MMSI (latched from AIVDO sentences). */
export let ownVesselMmsi: string | null = null;
...
export function parseNmeaLatitude(val: string, hemi: string): number | null {
  if (!val || !hemi) return null;
  val = val.trim();
  const upperHemi = hemi.trim().toUpperCase();
  if (upperHemi !== 'N' && upperHemi !== 'S') return null;
  ...
  if (isNaN(degs) || isNaN(mins)) return null;

  let lat = degs + mins / 60;
  if (upperHemi === 'S') lat = -lat;
  return lat;
}

/** Parses NMEA longitude field (DDDMM.MMM format). */
export function parseNmeaLongitude(val: string, hemi: string): number | null {
  if (!val || !hemi) return null;
  val = val.trim();
  const upperHemi = hemi.trim().toUpperCase();
  if (upperHemi !== 'E' && upperHemi !== 'W') return null;
  const dot = val.indexOf('.');
  if (dot < 0) return null;
  ...
  if (upperHemi === 'W') lon = -lon;
  return lon;
}

/** Formats coordinates to a display string, e.g. "N 41° 18.660'". */
export function formatCoords(lat: number, lng: number): { latStr: string; lngStr: string } {
  const formatComponent = (val: number, isLat: boolean): string => {
...
export function parseNmeaSentence(sentence: string): any {
  ...
  if (sentence.includes('AIVDM') || sentence.includes('AIVDO')) {
    const aisData = parseAisSentence(sentence);
    if (aisData) {
      return { aisData };
    }
  }
  ...
  if (type === 'DBT' || type === 'DBK' || type === 'DBS') {
    ...
    if (feetStr) {
      ...
      if (!isNaN(parsedFeet)) feet = parsedFeet;
    } else if (metersStr) {
      const meters = parseFloat(metersStr);
      if (!isNaN(meters)) feet = meters * 3.28084;
    }
    if (feet !== undefined) {
      return type === 'DBT'
  ...
  if (type === 'VWR') {
    ...
    if (speedKnotsStr) {
      ...
      if (!isNaN(parsed)) speed = parsed * 1.94384;
    }
...
export function handleNmeaSentence(sentence: string, liveData: NmeaLiveData): void {
  ...
  if (parsed.aisData && parsed.aisData.mmsi) {
    ...
    if (parsed.aisData.isOwnVessel && parsed.aisData.mmsi) {
      ownVesselMmsi = parsed.aisData.mmsi;
    }

    // Mark as own vessel if MMSI matches latched own MMSI
    const isOwn = Boolean(
      parsed.aisData.isOwnVessel ||
      (ownVesselMmsi && String(parsed.aisData.mmsi).trim() === String(ownVesselMmsi).trim())
    );

    const existing = liveAisTargetsMap.get(parsed.aisData.mmsi) || {};
    liveAisTargetsMap.set(parsed.aisData.mmsi, {
      ...existing,
      ...parsed.aisData,
      isOwnVessel: isOwn || Boolean((existing as any).isOwnVessel),
      lastSeen: Date.now()
    });
  }
