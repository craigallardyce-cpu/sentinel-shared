 ...
 */
import { parseAisSentence } from './ais.js';
...
export function parseNmeaLatitude(val, hemi) {
    if (!val || !hemi)
        return null;
    val = val.trim();
...
export function parseNmeaLongitude(val, hemi) {
    if (!val || !hemi)
        return null;
    val = val.trim();
    const upperHemi = hemi.trim().toUpperCase();
...
export function handleNmeaSentence(sentence, liveData) {
