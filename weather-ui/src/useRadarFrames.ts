...
export function useRadarFrames({
  radarEnabled = false
}: UseRadarFramesOptions = {}): UseRadarFramesResult {
  ...
  const loadRadarFrames = useCallback(async () => {
    try {
      ...
      const frames: RadarFrame[] = data.radar.past.map(
        (frame: { time: number; path: string }, index: number) => {
          ...
          const relativeText =
            index === data.radar.past.length - 1 ? 'Live' : `-${minutesAgo}m`;
          return {
            time: frame.time,
            path: frame.path,
  ...
  const latestRadarFrame = useMemo(
    () => (radarFrames.length > 0 ? radarFrames[radarFrames.length - 1] : null),
    [radarFrames]
  );
