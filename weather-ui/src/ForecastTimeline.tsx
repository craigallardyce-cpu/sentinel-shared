          ...
          >
            ...
            <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-lg border ${gridBgClass}`}>
              <div className="flex flex-col space-y-1">
                <span className={`text-[8px] font-mono uppercase tracking-widest leading-none ${textMutedClass}`}>Max Wind (Inc Gusts)</span>
                <span className={`text-sm font-mono font-bold ${textPrimaryClass}`}>{period.windRange}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className={`text-[8px] font-mono uppercase tracking-widest leading-none ${textMutedClass}`}>Direction</span>
                <span className={`text-sm font-mono font-bold ${textPrimaryClass}`}>{period.windDirection || 'Variable'}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className={`text-[8px] font-mono uppercase tracking-widest leading-none ${textMutedClass}`}>Temperature</span>
                <span className={`text-sm font-mono font-bold ${textPrimaryClass}`}>{formatTempRangeString(period.tempRange, tempUnit)}</span>
              </div>
              <div className="flex flex-col space-y-1">
                <span className={`text-[8px] font-mono uppercase tracking-widest leading-none ${textMutedClass}`}>Precipitation</span>
        ...
        <div key={idx} className={`flex flex-col p-3 rounded-lg border space-y-1.5 text-left ${cardBgBorder}`}>
          <div className="flex items-center justify-between">
            <div className="flex flex-col min-w-0">
              <span className={`text-[8.5px] font-mono truncate uppercase font-bold ${textMutedClass}`}>{p.periodName}</span>
              <div className="flex items-center gap-1.5 mt-0.5">
                <Wind size={8} className={`${windIconClass} animate-pulse`} />
                <span className={`text-[10px] font-mono font-bold ${textPrimaryClass}`}>{p.windRange}</span>
              </div>
            </div>
            {p.windDirection && (
              <div className="flex flex-col items-end shrink-0">
                <span className={`text-[7px] font-mono uppercase font-bold ${textMutedClass}`}>Dir</span>
                <div className="flex items-center gap-1 mt-0.5">
                  <span className={`text-[10px] font-mono font-bold ${textSecondaryClass}`}>{p.windDirection}</span>
                  <div 
          ...
          {(p.tempRange || p.precipChance) && (
            <div className={`flex items-center justify-between pt-1.5 border-t text-[8px] font-mono ${borderDividerClass}`}>
