...
export function windBandRgb(speedKts: number, isLightBg?: boolean): string {
...
export function windBandColor(speedKts: number, isLightBg?: boolean): string {
 ...
 * in the top colour.
 */
export function windScaleGradient(maxKts = 40, isLightBg?: boolean): string {
  const pct = (kts: number) => Math.min(100, (kts / maxKts) * 100);
  const stops: string[] = [];
  for (const band of WIND_BANDS) {
    if (band.from >= maxKts) break;
...
export function windBandEdges(maxKts = 40): number[] {
