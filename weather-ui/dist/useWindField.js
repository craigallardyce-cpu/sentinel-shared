...
export function useWindField({ lat, lon, enabled = false, hoursNeeded = 120, cache = null }) {
    ...
    */
    const snappedLat = typeof lat === 'number' && Number.isFinite(lat) ? snap(lat) : null;
    const snappedLon = typeof lon === 'number' && Number.isFinite(lon) ? snap(lon) : null;
    const bounds = useMemo(() => snappedLat !== null && snappedLon !== null
        ? {
